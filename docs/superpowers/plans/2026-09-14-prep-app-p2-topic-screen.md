...
**Interfaces:**
- Produces: every token listed below, consumed by Tasks 2-9.
...
```bash
git add package.json bun.lock src/styles/variables.css
git commit -m "feat: install markdown/mermaid deps and add P2 design tokens"
```
...
### Task 3: `ReferenceBadge` component

The clickable inline highlighted span. Presentational only — receives the
matched text and a click handler, no matching logic of its own.
...
```bash
git add src/components/ReferenceBadge
git commit -m "feat: add ReferenceBadge component"
```
...
describe('FilterChips', () => {
  test('renders a button per tag and marks the active one', () => {
...
```bash
git add src/components/FilterChips
git commit -m "feat: add FilterChips component"
```

---

### Task 5: `AnswerBody` component — markdown + mermaid + reference highlighting
...
- Produces:
  ...
  ```
  Consumed by Task 7 (`QuestionCard`) and Task 6 (`ReferenceModal`).
...
to verify it against would be unverifiable, dead code. Task 9 (content seed)
...
### Task 6: `ReferenceModal` component
...
**Interfaces:**
- Consumes: `AnswerBody` (Task 5), `Reference` from `@/types/topic.types`,
  `t` from `@/utils/i18n`.
- Produces:
  ```ts
...
export function ReferenceModal({ reference, onClose }: ReferenceModalProps) {
  ...
  return (
    <div className="reference-modal__scrim" onClick={onClose}>
      <div className="reference-modal__card" onClick={(event) => event.stopPropagation()}>
        <div className="reference-modal__header">
          <span className="reference-modal__eyebrow">{t('reference.eyebrow')}</span>
          <button type="button" className="reference-modal__close" aria-label={t('reference.close')} onClick={onClose}>
            ×
          </button>
        </div>
...
**Interfaces:**
- Consumes: `AnswerBody` (Task 5), `Question`/`Reference` from
  `@/types/topic.types`.
- Produces:
  ```ts
...
- Consumes: `topicsConfig` from `@/topics.config`, `IconRail` +
  `RailSection` (existing), `FilterChips` (Task 4), `QuestionCard` (Task 7),
  `ReferenceModal` (Task 6), `t` from `@/utils/i18n`, `useParams`/`Link` from
  `react-router`.
...
### Task 9: Seed `topics/nodejs.ts` + `references/nodejs.ts`
...
- `TopicPage`'s not-found path, filter-reset-on-topic-change, and
  ...
  in Task 8 resets `activeFilter`/`openQuestionId` before the new topic's
  data lands, not after).
- `topics/nodejs.ts` content is verbatim against `data/nodejs.md` (spot-check
  several questions even if Task 9's review already passed).
