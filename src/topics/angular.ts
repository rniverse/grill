...
export const questions: Question[] = [
  ...
  },
  {
    ...
    related: [],
    answer: { type: 'markdown.text', value: "A Signal is a reactive container around a value that tracks exactly which template bindings read it. When it changes, Angular updates only those specific bindings — fine-grained reactivity — rather than walking the component tree. Observables model async *streams* over time (HTTP calls, WebSocket events, combining multiple sources); Signals model synchronous, glitch-free *state*. Rule of thumb interviewers like: use Signals for local/component state, RxJS for async orchestration — and `toSignal()`/`toObservable()` bridge the two when needed." },
  },
  {
  ...
  },
  {
    ...
    related: [],
    answer: { type: 'markdown.text', value: "This is a common senior-level scenario question. A strong answer walks through: (1) whether change detection is running excessively — check if the component is default (checks whole tree) vs OnPush; (2) whether subscriptions are properly cleaned up in `ngOnDestroy` (leaks compound over time); (3) whether the `async` pipe is used instead of manual subscribe+setState (avoids manual CD triggers); (4) profiling with Chrome DevTools' Performance tab or Angular DevTools to see change-detection cycle frequency; (5) whether `trackBy` is used on any `*ngFor`/`@for` rendering list items from those streams." },
  },
  {
  ...
  },
  {
    ...
    references: [],
    related: [],
    answer: { type: 'markdown.text', value: "`providedIn: 'root'` creates one singleton shared by the whole app (and enables tree-shaking if unused). Providing in a component's own `providers` array creates a new instance for that component subtree — useful when you want isolated state per component instance (e.g., a wizard step, a modal)." },
  },
  {
  ...
  },
  {
    ...
    references: [],
    related: [],
    answer: { type: 'markdown.text', value: "Attach a custom validator function to the parent `FormGroup` (not the individual controls), since it needs access to sibling controls' values — e.g., comparing `password` and `confirmPassword` and returning a validation error object on the group if they don't match." },
  },
  {
    ...
    references: [],
    related: [],
    answer: { type: 'markdown.text', value: "Functional guards are plain functions (e.g., `CanActivateFn`) that use `inject()` to pull in dependencies, replacing the older pattern of implementing a `CanActivate` interface on an injectable class. They're less boilerplate-heavy and align with the standalone-components direction of the framework." },
  },
  {
  ...
  },
  {
    ...
    references: [signalRef.id],
    related: [],
    answer: { type: 'markdown.text', value: "Every change-detection cycle re-evaluates template expressions, so a method call there re-executes on every cycle — potentially many times per second — even if its inputs haven't changed. The fix is a pure `pipe`, a memoized `computed()` Signal, or precomputing the value and binding to a property instead." },
  },
  {
  ...
  },
  {
    ...
    references: [],
    related: [],
    answer: { type: 'markdown.text', value: "`TestBed` configures a testing module (or standalone component's testing environment) so Angular can create the component with its dependencies resolved — real or mocked. Typical pattern: provide a mock/stub for services via `TestBed.configureTestingModule({ providers: [{ provide: RealService, useValue: mockService }] })`, then assert on the component's rendered output or emitted events." },
  },
  {
  ...
  },
  {
    ...
    references: [],
    related: [],
    answer: { type: 'markdown.text', value: `This is an open scenario prompt: describe a real performance problem you diagnosed in an Angular app, and what you changed. There's no single expected fix here — the interviewer wants your own debugging story. ${scenarioStructureFraming}` },
  },
  {
  ...
  },
  {
    ...
    references: [],
    related: [],
    answer: { type: 'markdown.text', value: `This is an open scenario prompt: when would you deliberately *not* use Signals, even in a green-field v18+ app? There's no single expected answer — the interviewer wants your own reasoning about the trade-off, not a rule you memorized. ${scenarioStructureFraming}` },
  },
  {
    ...
    references: [],
    related: [],
    answer: { type: 'markdown.text', value: `This is an open scenario prompt: a component re-renders 40 times a second on a production dashboard, and you're asked to walk through your investigation. There's no single expected fix — the interviewer wants to hear how you'd narrow it down, not the answer itself. ${scenarioStructureFraming}` },
  },
  {
    ...
    references: [],
    related: [],
    answer: { type: 'markdown.text', value: `This is an open scenario prompt: how do you decide what should live in a Signal vs an Observable vs an NgRx store in a medium-sized app? There's no single expected answer — the interviewer wants your own criteria for that trade-off. ${scenarioStructureFraming}` },
  },
]
